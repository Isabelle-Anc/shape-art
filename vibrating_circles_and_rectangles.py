...
class Rect:
	def __init__ (self, x, y):
		...
		self.color = '#{0:0>6x}'.format(random.randint(00,16**6))
		self.length = random.randint(5,40)
		self.width = random.randint(75,200)
	...
	def draw(self, canvas):
		...
		canvas.create_rectangle(self.x, self.y, self.x + self.length, self.y + self.width, fill=self.color, outline="black")
